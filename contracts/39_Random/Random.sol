...
contract Random is ERC721, VRFConsumerBaseV2 {
    ...
    //
    VRFCoordinatorV2Interface COORDINATROR;
    ...
    constructor(
        ...
        COORDINATROR = VRFCoordinatorV2Interface(vrfCoordinator);
        subId = s_subId;
    }
    ...
    function pickRandomUniqueId(
        ...
        //随机数取模，得到tokenId, 作为数组下标，同时记录value为len-1, 如果取模得到的值已存在，
        //则tokenId取该数组下标的value
    ...
    /**
     * 调用VRF获取随机数，并mint NFT
     * 要调用requestRandomness()函数获取，消耗随机数的逻辑写在VRF的回调函数fulfillRandomness()中
     * 调用前，需要在Subscription中fund足够的Link
     */
    function mintRandomVRF() public {
        //调用requestRandomness获取随机数
        requestId = COORDINATROR.requestRandomWords(
            keyHash,
            subId,
